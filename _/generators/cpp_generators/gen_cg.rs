...
#[derive(Parser, Debug)]
#[command(name = "gen_cg")]
#[command(about = "Generate a C conjugate gradient solver program", long_about = None)]
struct Args {
    /// Matrix size (N x N)
...
fn main() {
    ...
    for i in 0..n {
        let rhs_matvec_i = format!(
            ...
            i,
            (0..n)
                .map(|j| format!("A_{}_{} * x_0_{}", i, j, j))
                .collect::<Vec<_>>()
                .join(" + ")
        );
        let lhs_matvec_i = format!("r_0_{}", i);
    ...
    // Write main function with example values
    fout.write_all(b"int main() {\n").unwrap();
    fout.write_all(b"\t// Initialize the input values (example identity matrix and ones vector)\n").unwrap();

    // Initialize b vector
    for i in 0..n {
        write!(fout, "\tdouble b_{} = 1.0;\n", i).unwrap();
    }

    // Initialize A matrix (identity for example)
    for i in 0..n {
        for j in 0..n {
            if i == j {
                write!(fout, "\tdouble A_{}_{} = 1.0;\n", i, j).unwrap();
            } else {
                write!(fout, "\tdouble A_{}_{} = 0.0;\n", i, j).unwrap();
            }
        }
    }
