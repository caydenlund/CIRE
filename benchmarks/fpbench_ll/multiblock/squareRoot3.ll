; ModuleID = 'benchmarks_c/squareRoot3.c'
source_filename = "benchmarks_c/squareRoot3.c"
...
entry:
  %cmp = fcmp olt double %x, 1.000000e-05
...
if.else:                                          ; preds = %entry
  ...
  %call = tail call double @sqrt(double noundef %add) #3, !tbaa !5
  br label %if.end

if.end:                                           ; preds = %if.else, %if.then
  %tmp.0 = phi double [ %0, %if.then ], [ %call, %if.else ]
  ret double %tmp.0
}
